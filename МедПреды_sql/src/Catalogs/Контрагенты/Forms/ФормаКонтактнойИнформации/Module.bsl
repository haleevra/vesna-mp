...
Функция ЗагрузитьНастройкиСоставаКИ()
	...
	Выборка = МедПредыФирмойСервер.ПолучитьДоступныеДляПечатиВидыКИ().Выбрать();
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ИспользованиеКИ = Неопределено Тогда
			ИспользованиеКИ = МедПредыФирмойСервер.УстановитьПечатьВидаКИПоУмолчанию(Выборка.ВидКИ);
		КонецЕсли;
