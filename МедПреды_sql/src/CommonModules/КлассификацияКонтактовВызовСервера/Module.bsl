...
Функция КонтрагентыСегментов(Сегменты) Экспорт
	...
	Для Каждого Сегмент Из Сегменты Цикл
		...
		Иначе
			Контрагенты = МедПредыКлиентСервер.ПолучитьСовпадающиеЭлементыМассивов(Контрагенты, СоставСегмента);
		КонецЕсли;
...
Функция КонтрагентыПоТегамИСегментам(Теги, Сегменты) Экспорт
	...
	Если Теги.Количество() > 0 И Сегменты.Количество() > 0 Тогда
		Контрагенты = МедПредыКлиентСервер.ПолучитьСовпадающиеЭлементыМассивов(КонтрагентыПоТегам(Теги), КонтрагентыСегментов(Сегменты));
