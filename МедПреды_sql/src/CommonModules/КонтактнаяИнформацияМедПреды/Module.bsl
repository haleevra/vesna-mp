...
Процедура ОбновитьЭлементыКонтактнойИнформации(Форма, ШиринаПоляВида = 8) Экспорт
	...
	Если КомандыДобавления <> Неопределено Тогда
		КомандыДобавления.Видимость = КонтактнаяИнформацияМедПредыКлиентСервер.СписокВидовДляДобавленияКонтактнойИнформации(Форма).Количество() > 0;
	КонецЕсли;
	...
	КонтактнаяИнформацияМедПредыКлиентСервер.ЗаполнитьСписокВыбораАдресов(Форма);
...
Процедура Контрагенты_УстановитьСвойстваВидов()
	...
	КонтактнаяИнформацияМедПреды.УстановитьФлагВыводитьВФормеВсегда(ПараметрыВида.Вид);
...
Процедура Пользователи_УстановитьСвойстваВидов()
	...
	ПараметрыВида = УправлениеКонтактнойИнформацией.ПараметрыВидаКонтактнойИнформации("Адрес");
	ПараметрыВида.Вид = Справочники.ВидыКонтактнойИнформации.АдресПользователя;
	ПараметрыВида.Порядок = 3;
	ПараметрыВида.МожноИзменятьСпособРедактирования		= Истина;
	ПараметрыВида.РедактированиеТолькоВДиалоге			= Ложь;
	ПараметрыВида.ОбязательноеЗаполнение				= Ложь;
...
Процедура ИнициализироватьФорму(Форма)
	...
	Если КонтактнаяИнформацияМедПредыКлиентСервер.СписокВидовДляДобавленияКонтактнойИнформации(Форма).Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Команда.Действие = "Подключаемый_КонтактнаяИнформацияМедПредыВыполнитьКоманду";
