...
Процедура ПередДобавлениемПодсистемы(Описание) Экспорт
	...
	Если ВРег(Метаданные.Имя) = ВРег("МедПредыБазовая") Тогда
		
		Описание.Имя = "МедПредыБазовая";
		
	Иначе
		
		Описание.Имя = "МедПреды";
		
	КонецЕсли;
	
КонецПроцедуры

Функция ЭтоЗаказчик() Экспорт
...
// Комментирует строки запроса от {ТэгЭкранирования до //}
// Пример:
// ВЫБРАТЬ 
...
// ВЫБРАТЬ 
// 		Номенклатура.Ссылка КАК Номенклатура
//	 	Характеристика.Ссылка КАК Характеристика
// ИЗ
//		 Справочник.Номенклатура КАК Номенклатура
// ЛЕВОЕ СОЕДИНЕНИЕ
//		Справочник.ХарактеристикиНоменклатуры КАК Характеристики
// ПО
// 		Номенклатура.Ссылка = Характеристики.Владелец
// //}
//
// В результате будет возвращён запрос
//
// ВЫБРАТЬ 
// 		Номенклатура.Ссылка КАК Номенклатура
//	 	Значение(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) КАК Характеристика
...
// //		Номенклатура.Ссылка = Характеристики.Владелец
// //}
Функция ЭкранироватьТекстЗапроса(ТекстЗапроса, ТэгЭкранирования) Экспорт
	
	НомерТэга = 1;
	РезультирующийЗапрос = ТекстЗапроса;
	ПозицияКонцаЭкранирования = 0;
	// ищем вхождение всех тэгов
	Пока Истина Цикл
		
		ПозицияНачалаЭкранирования = СтрНайти(РезультирующийЗапрос, "//{"+ТэгЭкранирования+Символы.ПС,,ПозицияКонцаЭкранирования+1);
		
		// больше тэгов не найдено
		Если ПозицияНачалаЭкранирования = 0 Тогда 
			Прервать;
		КонецЕсли;
		
		ПозицияКонцаЭкранирования = СтрНайти(РезультирующийЗапрос, "//}",,ПозицияНачалаЭкранирования);
		
		// если не найдено окончание блока экранирования, вызываем исключение
		Если ПозицияКонцаЭкранирования = 0 Тогда
			ВызватьИсключение(Нстр("ru='Не найдено окончание экранирования //}'"));
		КонецЕсли;
		
		РезультирующийЗапросВрем = Лев(РезультирующийЗапрос, ПозицияНачалаЭкранирования-1);
		
		// участок запроса, подлежащий экранированию
		ЭкранируемыйЗапрос = Сред(
			РезультирующийЗапрос,
			ПозицияНачалаЭкранирования,
			ПозицияКонцаЭкранирования - ПозицияНачалаЭкранирования + СтрДлина("//}"));
		
		РезультирующийЗапросВрем = РезультирующийЗапросВрем + СтрЗаменить(ЭкранируемыйЗапрос, Символы.ПС, Символы.ПС+"//");
		
		РезультирующийЗапросВрем = РезультирующийЗапросВрем + Сред(РезультирующийЗапрос, ПозицияКонцаЭкранирования+СтрДлина("//}"));
		
		НомерТэга = НомерТэга + 1;
		РезультирующийЗапрос = РезультирующийЗапросВрем;
	КонецЦикла;
	
	Если НомерТэга = 1 Тогда
		ВызватьИсключение(НСтр("ru='Тэг экранирования не обнаружен'"));
	КонецЕсли;
	
	Возврат РезультирующийЗапрос;
...
//
Функция ПолучитьДанныеКИКонтрагентаДляВыбораНаФорме_old(Контрагент, Элементы, ПоляФормыСодержащиеКИ) Экспорт
	
	ТипыКИ = Новый Массив;
	Для каждого Эл Из ПоляФормыСодержащиеКИ Цикл
		Элементы[Эл.Значение].СписокВыбора.Очистить();
		ТипыКИ.Добавить(Эл.Ключ);
	КонецЦикла;
	...
	ВладельцыКИ = Запрос.Выполнить().Выгрузить();
	ВладельцыКИМассив = Новый Массив;
	
	Для каждого ВладелецКИ Из ВладельцыКИ Цикл
		ВладельцыКИМассив.Добавить(ВладелецКИ.Ссылка);
	КонецЦикла;
	
	ОсновноеКЛ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "КонтактноеЛицо");
	КонтактнаяИнформацияОбъектов = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(ВладельцыКИМассив, ТипыКИ);
	...
	Для каждого ВладелецКИ Из ВладельцыКИ Цикл
		...
		Для каждого Эл Из ПоляФормыСодержащиеКИ Цикл
			...
			СтрокиКИ = КонтактнаяИнформацияОбъектов.НайтиСтроки(Новый Структура("Объект,Тип", ВладелецКИ.Ссылка, Эл.Ключ));
			Если СтрокиКИ.Количество() > 0 Тогда
				ПолеФормы = Элементы[ПоляФормыСодержащиеКИ.Получить(Эл.Ключ)];
				ПолеФормы.СписокВыбора.Добавить("", ПредставлениеВладелельцаКИ);
				Для каждого ЗначениеКИ Из СтрокиКИ Цикл
					ПредставлениеКИ = Новый ФорматированнаяСтрока(
						...
						ЗначениеКИ.Представление
					);
					ПолеФормы.СписокВыбора.Добавить(ЗначениеКИ.Представление, ПредставлениеКИ);
				КонецЦикла;
			КонецЕсли;
	...
	Для каждого Эл Из ПоляФормыСодержащиеКИ Цикл
		ЗначениеКИ = "";
		Если ЗначениеЗаполнено(ОсновноеКЛ) Тогда
			ОтборСтрок = Новый Структура("Объект,Тип", ОсновноеКЛ, Эл.Ключ);
			СтрокиКИ = КонтактнаяИнформацияОбъектов.НайтиСтроки(ОтборСтрок);
			Если СтрокиКИ.Количество() <> 0 Тогда
				ЗначениеКИ = СтрокиКИ[0].Представление;
			КонецЕсли;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ЗначениеКИ) Тогда
			ОтборСтрок = Новый Структура("Объект,Тип", Контрагент, Эл.Ключ);
			СтрокиКИ = КонтактнаяИнформацияОбъектов.НайтиСтроки(ОтборСтрок);
			Если СтрокиКИ.Количество() <> 0 Тогда
				ЗначениеКИ = СтрокиКИ[0].Представление;
			КонецЕсли;
		КонецЕсли;
		ЗначенияКИпоУмолчанию.Вставить(Эл.Ключ, ЗначениеКИ);
	КонецЦикла;
...
Функция ПолучитьДанныеКИКонтрагентаДляВыбораНаФорме(Контрагент, Элементы, ПоляФормыСодержащиеКИ) Экспорт
	...
	|	КонтактныеЛица.РеквизитДопУпорядочивания КАК РеквизитДопУпорядочивания,
	...
	|	И КонтактныеЛица.КонтактнаяИнформация.Тип В(&ТипыКИ)
	|
	|УПОРЯДОЧИТЬ ПО
	|	РеквизитДопУпорядочивания
	|;
...
Функция ЭтоПолноправныйПользователь(Пользователь = Неопределено,
	...
	// МедПредыСервер.ПользователюУстановленПрофиль(, , ПредопределенноеЗначение("Справочник.ПрофилиГруппДоступа.Администратор"))
...
Процедура ИнициализироватьДополнительныеСвойстваДляПроведения(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	СтруктураДополнительныеСвойства.ДляПроведения.Организация = МедПредыСервер.ПолучитьОрганизацию(СтруктураДополнительныеСвойства.ДляПроведения.Организация);
...
Процедура ФамилияИнициалыПоНаименованию(ФИО, Наименование) Экспорт
	...
	ФИО = МедПредыСервер.ПолучитьФамилиюИмяОтчество(Фамилия, Имя, Отчество, Истина);
...
Функция ПолучитьКонтактнуюИнформацию(ОбъектКонтактнойИнформации, ВидИнформации) Экспорт
	...
	ИначеЕсли ТипЗнч(ОбъектКонтактнойИнформации) = Тип("СправочникСсылка.КонтактныеЛица") Тогда
		
		ТаблицаИсточник = "КонтактныеЛица";
...
//
//
Процедура ПроверитьВозможностьПечатиУПД(ПараметрКоманды, Ошибки) Экспорт
...
КонецПроцедуры // ПроверитьВозможностьПечатиУПД()
...
Функция ПодготовитьСтруктуруДанныхПечатиЭтикетокИЦенниковИзДокументов(МассивДокументов, ЭтоЦенники) Экспорт
	...
	Запрос = Новый Запрос;
	// не применять конструктор запросов из-за наличия тегов
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Если НЕ ПравоДоступа("Чтение",Метаданные.Документы.СборкаЗапасов) Тогда
		Запрос.Текст = ЭкранироватьТекстЗапроса(Запрос.Текст, "СборкаЗапасов");
	КонецЕсли;
	
	Если НЕ ПравоДоступа("Чтение",Метаданные.Документы.ПеремещениеЗапасов) Тогда
		Запрос.Текст = ЭкранироватьТекстЗапроса(Запрос.Текст, "ПеремещениеЗапасов");
	КонецЕсли;
	
	Если НЕ ПравоДоступа("Чтение",Метаданные.Документы.ОприходованиеЗапасов) Тогда
		Запрос.Текст = ЭкранироватьТекстЗапроса(Запрос.Текст, "ОприходованиеЗапасов");
	КонецЕсли;
	...
	СтруктураДействий.Вставить("ЗаполнитьОрганизацию", ?(МассивОрганизаций.Количество() = 1,МассивОрганизаций[0],
		МедПредыПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(ПользователиКлиентСервер.ТекущийПользователь(), "ОсновнаяОрганизация")));
	...
	СтруктураДействий.Вставить("ЗаполнитьВидЦен", ?(МассивВидовЦен.Количество() = 1,МассивВидовЦен[0],
		МедПредыПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(ПользователиКлиентСервер.ТекущийПользователь(), "ОсновнойВидЦенПродажи")));
...
Процедура УстановитьТекстПроСчетФактуру(ФормаДокумента, Полученный = Ложь) Экспорт
	...
	НайденныйСчетФактура = МедПредыСервер.ПолучитьПодчиненныйСчетФактуру(ФормаДокумента.Объект.Ссылка, Полученный);
...
Процедура ИзменитьПодчиненнуюСчетФактуру(ДокументОснование, Полученный = Ложь) Экспорт
	...
	СчетФактура = МедПредыСервер.ПолучитьПодчиненныйСчетФактуру(ДокументОснование, Полученный);
...
Процедура ОпределитьИсточникиПечатиУПД(ПараметрКоманды, ИсточникиПечати, СообщенияПользователю)
	...
	Для каждого ДокументПечати Из ПараметрКоманды Цикл
		...
		СчетФактураВыданный = МедПредыСервер.ПолучитьПодчиненныйСчетФактуру(ДокументПечати, Ложь);
...
Функция ПроверитьВыпискуСчетовФактур(ИсточникиПечати, ДокументРеализации, СообщенияПользователю, ВидОперации = Неопределено)
	...
	СчетФактураВыданный = МедПредыСервер.ПолучитьПодчиненныйСчетФактуру(ДокументРеализации, Ложь);
...
Процедура ЗаполнитьРасшифровкуПлатежаРасход(ТекущийОбъект, Компания = Неопределено, СтавкаНДСПоУмолчанию = Неопределено, Курс = Неопределено, Кратность = Неопределено, Договор = Неопределено) Экспорт
	...
	Если Компания = Неопределено Тогда
		Компания = МедПредыСервер.ПолучитьОрганизацию(ТекущийОбъект.Организация);
	КонецЕсли;
	...
	Если СтавкаНДСПоУмолчанию = Неопределено Тогда
		...
		ИначеЕсли ТекущийОбъект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС Тогда
			СтавкаНДСПоУмолчанию = МедПредыПовтИсп.ПолучитьСтавкуНДСБезНДС();
		Иначе
			СтавкаНДСПоУмолчанию = МедПредыПовтИсп.ПолучитьСтавкуНДСНоль();
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		НуженОтборПоДоговорам = МедПредыПовтИсп.ТребуетсяКонтрольДоговоровКонтрагентов();
	...
	Пока СуммаОсталосьРаспределить > 0 Цикл
		...
		Если ВыборкаРезультатаЗапроса.Следующий() Тогда
			...
			Иначе // сумма остатка больше чем нужно распределить
				...
				НоваяСтрока.СуммаРасчетов = МедПредыСервер.ПересчитатьИзВалютыВВалюту(
